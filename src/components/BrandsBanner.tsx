  ...
  const brandsList = [
    {
      name: "PVR INOX",
      content: (
        <div className="flex flex-col items-center justify-center p-6 min-w-[240px]">
        ...
        <div className="relative w-full overflow-hidden mb-16 py-4 mask-gradient">
          ...
          <div className="flex w-max animate-marquee space-x-12">
            ...
            {brandsList.map((brand, idx) => (
              <div key={`brand-1-${idx}`} className="flex-shrink-0 bg-[#081325]/30 border border-white/5 rounded-2xl hover:border-brand-blue/30 transition-all duration-300">
                {brand.content}
              </div>
            ))}
            {/* Duplicate list loop for loop continuity */}
            {brandsList.map((brand, idx) => (
              <div key={`brand-2-${idx}`} className="flex-shrink-0 bg-[#081325]/30 border border-white/5 rounded-2xl hover:border-brand-blue/30 transition-all duration-300">
                {brand.content}
              </div>
            ))}
          </div>
